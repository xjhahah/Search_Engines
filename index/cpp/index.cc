...
bool Index::Build(const std::string& input_path) {
  ...
  //    打开的文件其实是 raw_input 文件. 这个文件是预处理
  //    之后得到的行文本文件. 每行对应一个 html. 
...
void Index::BuildInverted(const DocInfo& doc_info) {
  // 构造倒排索引的核心流程
  // 0. 先统计好 doc_info 中每个词在标题和正文中出现的次数
...
int32_t Index::CalcWeight(const WordCnt& word_cnt) {
  // 此处的权重计算公式, 拍脑门出来的.
  // 实际上在真实的商业搜索中, 此处的权重计算过程会
  // 非常复杂
  return 10 * word_cnt.first_pos + word_cnt.content_cnt;
}
